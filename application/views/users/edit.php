...
$page_subtitle = 'Manage account access, identity details, and password from one place.';
$submit_label = 'Update Account';
$form_action = site_url("users/update_submit/" . $this->uri->segment(3));
...
<div class="col-lg-12 grid-margin stretch-card user-editor">
	<div class="card user-editor-card">
		<div class="user-editor-hero">
			<h2><i class="mdi mdi-account-edit"></i> <?= $page_title ?></h2>
			<p><?= $page_subtitle ?></p>
		</div>
			...
			<form action="<?= $form_action ?>" method="POST">
				<div class="user-section">
					...
					<div class="row">
						...
						<div class="col-md-6 user-field">
							...
							<?= form_dropdown('usertype', $user_types, set_value('usertype', $row->usertype), 'class="form-control"') ?>
						</div>
					</div>
				</div>
				...
				<div class="user-section">
					...
					<div class="row">
						<div class="col-md-6 user-field">
							<label>E-mail</label>
							<input type="text" name="emailadd" value="<?= set_value('emailadd',$row->emailadd) ?>" class="form-control" />
						</div>
						<div class="col-md-6 user-field">
							<label>Birthdate</label>
							<input type="date" name="birthdate" value="<?= set_value('birthdate', !empty($row->birthdate) ? date('Y-m-d', strtotime($row->birthdate)) : '') ?>" class="form-control" />
						</div>
					</div>
				</div>
				...
				<div class="user-section">
					...
					<div class="row">
						<div class="col-md-6 user-field">
							...
							<?= form_dropdown('gradelevel', $grade_options, set_value('gradelevel',$row->gradelevel), 'class="form-control"') ?>
						</div>
						<div class="col-md-6 user-field">
							<label>Secondary Grade Level</label>
							<?= form_dropdown('gradelevel1', $grade_options, set_value('gradelevel1',$row->gradelevel1), 'class="form-control"') ?>
						</div>
					</div>
				</div>
				...
				<div class="user-section">
					...
					<p class="user-note">Leave both password fields blank if you do not want to change the current password.</p>
					<div class="row">
						<div class="col-md-6 user-field">
