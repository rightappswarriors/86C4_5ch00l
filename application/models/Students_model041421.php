...
class Students_model extends CI_Model
{
	...
	function students_list()
	{
		...
		$query = "select a.*,b.id as enroll_id,b.ableforpt, b.newold,b.gradelevel,b.status as enrollstatus, c.firstname as user_firstname, c.lastname as user_lastname, c.lastlogin as user_lastlogin, c.mobileno as user_mobileno from students a join enrolled b on b.studentid = a.id left join register c on c.id = a.user_id where b.deleted = 'no'".$parent_search." and b.schoolyear = ".$this->session->userdata('current_schoolyearid').$withstatus." order by b.id asc ";
		return $this->db->query( $query );	
	}
	...
	function count_reenrollments(){
		return $this->db->query("select count(*) as num_reenrolls from preenrollstudents where status = '1' and schoolyear = ".$this->session->userdata('current_schoolyearid')." group by studentid");	
	}
