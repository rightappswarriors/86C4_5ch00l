...
class Login extends CI_Controller {
	...
	public function resend_code()
	{
		...
		if (!$identifier) {
			echo json_encode(array('status' => 'error', 'message' => 'Session expired. Please try again.'));
			return;
		}
	...
	/**
	 * Show help page for Gmail App Password
	 */
	public function app_password_help()
	{
		$this->load->view('app_password_help');
	}
	
	/**
	 * Save Gmail App Password
	 */
	public function save_app_password()
	{
		$app_password = $this->input->post('app_password');
		
		if (empty($app_password) || strlen($app_password) < 10) {
			$this->session->set_flashdata('message', 'Invalid App Password');
			redirect('login/app_password_help');
			return;
		}
		
		// Read current config
		$config_file = APPPATH . 'config/email_sms_config.php';
		...
		if (file_put_contents($config_file, $content)) {
			$this->session->set_flashdata('success', 'Gmail App Password saved! Email sending is now enabled.');
		} else {
			$this->session->set_flashdata('message', 'Failed to save password. Please check file permissions.');
		}
