...
class Students_model extends CI_Model
{
	...
	function students_list()
	{
		...
		if($this->session->userdata('current_usertype') != 'Parent'){
			...
			$query = "select a.*,b.gradelevel,b.status as enrollstatus, c.firstname as user_firstname, 
			...
			where b.deleted = 'no' and b.schoolyear = ".$this->session->userdata('current_schoolyearid').$withstatus. " order by b.id desc ";
		}
	...
	function students_list_dashboard()
	{
		...
		//if($this->session->userdata('current_usertype') == 'Registrar' || $this->session->userdata('current_usertype') == 'Admin'){
			...
			join enrolled b on b.studentid = a.id 
			left join register c on c.id = a.user_id 
			where b.deleted = 'no' and b.schoolyear = ".$this->session->userdata('current_schoolyearid'). " order by b.id desc limit 5";
		//}
	...
	{
		$query = "select a.*,b.gradelevel,b.newold from students a left join enrolled b on b.studentid = a.id where a.id = $id and b.deleted = 'no'";	
		return $this->db->query($query);
	}
	
	function count_newold_students(){
		return $this->db->query("select count(*) as num_newold, newold from enrolled where deleted = 'no' group by newold");
	}
	...
	function count_gradelevel_students(){
		return $this->db->query("select count(*) as num_gradelevel, gradelevel from enrolled where deleted = 'no' group by gradelevel");
	}
